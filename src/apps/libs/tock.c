...
int subscribe(driver_t driver, uint32_t subscribe,
              subscribe_cb cb, void* userdata) {
  return __subscribe(driver.num, subscribe, cb, userdata);
}


int command(driver_t driver, uint32_t command, int data) {
  return __command(driver.num, command, data);
}

int allow(driver_t driver, uint32_t allow, void* ptr, size_t size) {
  return __allow(driver.num, allow, ptr, size);
}
